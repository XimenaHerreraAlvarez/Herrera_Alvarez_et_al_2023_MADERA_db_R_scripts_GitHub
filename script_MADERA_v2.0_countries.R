...
Bolivia_tnrs <- TNRS(taxonomic_names = Bolivia_submitted, sources = c("wcvp", "wfo"))
Bolivia_tnrs
...
Brazil_tnrs <- TNRS(taxonomic_names = Brazil_submitted, sources = c("wcvp", "wfo"))
Brazil_tnrs    
...
Colombia_tnrs <- TNRS(taxonomic_names = Colombia_submitted, sources = c("wcvp", "wfo"))                    
Colombia_tnrs
...
Ecuador_tnrs <- TNRS(taxonomic_names = Ecuador_submitted, sources = c("wcvp", "wfo"))                    
Ecuador_tnrs
...
French_Guiana_tnrs <- TNRS(taxonomic_names = French_Guiana_submitted1, sources = c("wcvp", "wfo"))                    
French_Guiana_tnrs
...
Guyana_tnrs <- TNRS(taxonomic_names = Guyana_submitted, sources = c("wcvp", "wfo"))                    
Guyana_tnrs
...
Herrero_Jauregui_tnrs <- TNRS(taxonomic_names = Herrero_Jauregui_submitted, sources = c("wcvp", "wfo"))                    
...
Herrero_Jauregui_tnrs1
table(Herrero_Jauregui_tnrs1$Taxonomic_status)

Peru_tnrs <- TNRS(taxonomic_names = Peru_submitted, sources = c("wcvp", "wfo"))                    
Peru_tnrs
...
Piponiot_tnrs <- TNRS(taxonomic_names = Piponiot_submitted, sources = c("wcvp", "wfo"))                    
Piponiot_tnrs
...
Suriname_tnrs <- TNRS(taxonomic_names = Suriname_submitted, sources = c("wcvp", "wfo"))                    
Suriname_tnrs
...
Venezuela_tnrs <- TNRS(taxonomic_names = Venezuela_submitted, sources = c("wcvp", "wfo"))                   
Venezuela_tnrs
...
Cardoso_tnrs <- TNRS(taxonomic_names = Cardoso_Amazonian_trees_submitted, sources = c("wcvp", "wfo"))
Cardoso_tnrs
...
#THE TOTAL NUMBER OF ACCEPTED SPECIES IS 1435 (does not include genera)

write.csv(Amazonian_countries_final,"Amazonian_countries_final_1435.csv")
...
#THE TOTAL NUMBER OF AMAZONIAN TIMBER ACCEPTED SPECIES IS 1138 (we don't include genera but we excluded two extra life forms, so we had 1136 spp)

Amazonian_timber

write.csv(Amazonian_timber,"Amazonian_timber_1138.csv")
